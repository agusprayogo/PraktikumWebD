	...
	?>
		...
		<tr>
			<td>
				<form method="post">
					urut berdasar:
					<input type="radio" name="fildosen" value="nip">NIP
					<input type="radio" name="fildosen" value="nidn">NIDN
					<input type="radio" name="fildosen" value="nama">nama
					<input type="radio" name="fildosen" value="email">email
					<button type="submit">urut</button>
				</form>
			</td>
		</tr>
		...
		$conn = mysqli_connect('localhost','root','','sorfil');
		$str='';
		if (isset($_POST['namadosen']) and $_POST['namadosen']!='') {
			$str=$str.' where nama like "%'.$_POST['namadosen'].'%"';
		}
		if (isset($_POST['fildosen'])) {
			$str=$str.' order by '.$_POST['fildosen'].'';
		}
